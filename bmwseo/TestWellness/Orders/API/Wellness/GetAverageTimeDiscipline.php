...
try {
    ...
    if ($activityName === 'Workout' || $activityName === 'Weight') {
        ...
        $query = "SELECT DATE_FORMAT(SEC_TO_TIME(AVG(TIME_TO_SEC(PostedDateTime))), '%h:%i %p') AS AvgPostedTime FROM data_table WHERE EntryUser = :userName AND ActivityName = :activityName AND Validity = 1 AND EntryDate >= :fromDatePDO AND EntryDate <= :currentDatePDO";
    } else {
        // For other activities, query the activity_table
        $query = "SELECT DATE_FORMAT(SEC_TO_TIME(AVG(TIME_TO_SEC(Time(PostedDateTime)))), '%h:%i %p') AS AvgPostedTime FROM activity_table WHERE EntryUser = :userName AND ActivityName = :activityName AND Validity = 1 AND EntryDate >= :fromDatePDO AND EntryDate <= :currentDatePDO";
    }
