...
try {
    ...
    if (trim($_GET['JsonUserName']) != "") {
        ...
        $stmt = $pdo->prepare("UPDATE `team_table` SET `Status` = 'Active', `CoachApproved` = '1' WHERE `UserName` = :usernamePDO");
        ...
        echo json_encode("Team member status updated successfully.");
    } else {
