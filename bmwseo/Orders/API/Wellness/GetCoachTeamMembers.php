...
try {
    ...
    if(!isset($_GET['JsonCountBoolean'])){
        ...
        $stmt = $pdo->prepare("SELECT `UserName` FROM `team_table` WHERE (`CoachName` = :coachname OR `CoCoachName` = :coachname) AND `Status` = 'Inactive' GROUP BY `UserName` ORDER BY `UserName`");
        $stmt->bindParam(':coachname', $CoachName, PDO::PARAM_STR);
        $stmt->execute();
        $results = $stmt->fetchAll(PDO::FETCH_ASSOC);
