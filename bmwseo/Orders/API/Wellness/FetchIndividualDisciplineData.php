...
try {
    ...
    if (trim($_GET['JsonDays']) != "") {
        ...
        switch ($activity) {
            ...
            case 'Breakfast':
                 // Breakfast query
                $stmt = $pdo->prepare("SELECT COUNT(*) AS EntryCount FROM activity_table WHERE EntryDate >= :daysPDO AND EntryDate <= :currentDatePDO AND ActivityName = 'Breakfast' AND Time(PostedDateTime) >= '05:00:00' AND Time(PostedDateTime) <= '08:30:59' AND Validity = 1 AND EntryUser LIKE :userPDO");
                $stmt->bindParam(':daysPDO', $Days);
                $stmt->bindParam(':userPDO', $UserName);
                $stmt->bindParam(':currentDatePDO', $currentDate);
                $stmt->execute();
            ...
            case 'Lunch':
                // Lunch query
                $stmt = $pdo->prepare("SELECT COUNT(*) AS EntryCount FROM activity_table WHERE EntryDate >= :daysPDO AND EntryDate <= :currentDatePDO AND ActivityName = 'Lunch' AND Time(PostedDateTime) >= '12:00:00' AND Time(PostedDateTime) <= '14:00:59' AND Validity = 1 AND EntryUser LIKE :userPDO");
                $stmt->bindParam(':daysPDO', $Days);
                $stmt->bindParam(':userPDO', $UserName);
                $stmt->bindParam(':currentDatePDO', $currentDate);
                $stmt->execute();
            ...
            case 'Dinner':
                // Dinner query
                $stmt = $pdo->prepare("SELECT COUNT(*) AS EntryCount FROM activity_table WHERE EntryDate >= :daysPDO AND EntryDate <= :currentDatePDO AND ActivityName = 'Dinner' AND Time(PostedDateTime) >= '17:30:00' AND Time(PostedDateTime) <= '20:30:59' AND Validity = 1 AND EntryUser LIKE :userPDO");
                $stmt->bindParam(':daysPDO', $Days);
                $stmt->bindParam(':userPDO', $UserName);
                $stmt->bindParam(':currentDatePDO', $currentDate);
                $stmt->execute();
            ...
            case 'Weight':
                // Weight query
                $stmt = $pdo->prepare("SELECT COUNT(*) AS EntryCount FROM data_table WHERE EntryDate >= :daysPDO AND EntryDate <= :currentDatePDO AND ActivityName = 'Weight' AND Time(PostedDateTime) >= '03:00:00' AND Time(PostedDateTime) <= '06:30:59' AND Validity = 1 AND EntryUser LIKE :userPDO");
                $stmt->bindParam(':daysPDO', $Days);
                $stmt->bindParam(':userPDO', $UserName);
                $stmt->bindParam(':currentDatePDO', $currentDate);
                $stmt->execute();
