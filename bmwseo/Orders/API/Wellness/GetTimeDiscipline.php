...
try {
    $Days = $_GET['JsonDays'];
    $UserName = isset($_GET['JsonUserName']) ? $_GET['JsonUserName'] : null;
    ...
    if (trim($_GET['JsonDays']) != "") {
        ...
        // Calculating Weight
        $witStmt = $pdo->prepare("SELECT COUNT(*) AS EntryCount FROM data_table WHERE EntryDate >= :daysPDO AND EntryDate <= :currentDatePDO AND ActivityName = 'Weight' AND Time(PostedDateTime) >= '03:00:00' AND Time(PostedDateTime) <= '06:30:59' AND Validity = 1 AND EntryUser LIKE :userPDO");
        ...
        echo json_encode(round($activity_discipline_percentage, 1) . "%");
    }
