...
try {
    ...
    if (isset($_GET['OrderNumber'])) {
        ...
        $checkStmt = $pdo->prepare("SELECT COUNT(*) FROM order_table WHERE RateWiseOrderNumber = :PositiveRateWiseOrderNumber");
        ...
        if ($count > 0) {
            ...
            echo json_encode(['success' => false, 'message' => 'RateWiseOrderNumber is already occupied.', 'conflict' => true]);
            exit;
        }
        ...
        if ($stmt) {
            ...
            if ($rowCount > 0) {
                ...
                echo json_encode(['success' => false, 'message' => 'No order restored']);
            }
        } else {
            echo json_encode(['success' => false, 'message' => 'Invalid OrderNumber specified.']);
        }
    } else {
        echo json_encode(['success' => false, 'message' => 'OrderNumber not provided.']);
    }
