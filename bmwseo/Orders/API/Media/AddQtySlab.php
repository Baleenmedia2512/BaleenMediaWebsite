...
try {
    ...
    $checkstmt = $pdo->prepare("SELECT * FROM rates_qty_slab_table WHERE RateId = :RateIdPDO AND StartQty = :StartQtyPDO AND Width = :WidthPDO");
    $checkstmt->bindParam(":RateIdPDO", $RateId);
    $checkstmt->bindParam(":StartQtyPDO", $Qty);
    $checkstmt->bindParam(":WidthPDO", $Width);
    $checkstmt->execute();
    ...
    if(!$result){
        ...
        $stmt = $pdo->prepare("UPDATE rates_qty_slab_table SET EntryDateTime = CURRENT_TIMESTAMP, EntryUser = :EntryUserPDO, UnitPrice = :UnitPricePDO, Unit = :UnitPDO, `Validity` = 1, Width = :PDOWidth WHERE StartQty = :StartQtyPDO AND RateId = :RateIdPDO AND Width = :PDOWidth");
        $stmt->bindParam(":EntryUserPDO", $EntryUser);
        $stmt->bindParam(":RateIdPDO", $RateId);
        $stmt->bindParam(":StartQtyPDO", $Qty);
        $stmt->bindParam(":UnitPricePDO", $UnitPrice);
