...
try {
    ...
    // if($dbName === 'Grace Scans'){
        if($search_type === 'contact') {
            ...
            $sql = "SELECT DISTINCT ClientName, ClientContact, ID FROM client_table WHERE ClientContact LIKE :suggestion AND ClientName != '' AND Validity = 1 LIMIT 5";
        } else {
            // Prepare the SQL statement to fetch client names based on the name
            $sql = "SELECT DISTINCT ClientName, ClientContact, ID FROM client_table WHERE ClientName LIKE :suggestion AND Validity = 1 LIMIT 5";
        }
    // } else{
    //     if($search_type === 'contact') {
    //         // Prepare the SQL statement to fetch client names based on the contact
